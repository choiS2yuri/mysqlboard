            ...
            <div className="flex justify-center gap-x-5 mb-5">
                ...
                {page > 5 && <button className=' border px-1.5 py-1 rounded text-sm  ' onClick={prevPage}>이전</button>}
                {
                    Array(endPage - startPage + 1).fill(null).map((_,i)=>{
                        ...
                        return(
                            <button key={pageNumber} onClick={()=>{setPage(pageNumber); setIsActive(i);}} className={`bg-white border px-1.5 py-1 rounded text-sm  basis-8 ${page === pageNumber ? "bg-pink-400 text-white" : " bg-white"}`}> {pageNumber} </button>   
                            )
                    })
                }
